...
class ServiceService {
    ...
    /**
     * Fetches all records from a given service table.
    ...
    public function getAll(string $tableName): string {
        if (!$this->isValidTable($tableName)) {
            ...
            return json_encode(['error' => 'Invalid service type specified.']);
        }
    ...
    /**
     * Creates a new record in a service table.
    ...
    public function create(string $tableName, ?string $body): string {
        ...
        try {
            ...
            return json_encode(['message' => 'service item created successfully.', 'id' => $id, 'name' => trim($data->name)]);
        } catch (Exception $e) {
    ...
    /**
     * Updates an existing record in a service table.
     * @param string $tableName The name of the table.
    ...
    public function update(string $tableName, int $id, ?string $body): string {
        ...
        try {
            ...
            if ($stmt->rowCount() === 0) {
                 ...
                 return json_encode(['error' => 'Service item not found or no changes made.']);
            }

            return json_encode(['message' => 'Service item updated successfully.']);
        } catch (Exception $e) {
            http_response_code(500);
            return json_encode(['error' => 'Database error: ' . $e->getMessage()]);
        }
    ...
    /**
     * Deletes a record from a service table.
     * @param string $tableName The name of the table.
    ...
    public function delete(string $tableName, int $id): string {
        ...
        try {
            ...
            if ($stmt->rowCount() === 0) {
                 http_response_code(404);
                 return json_encode(['error' => 'Service item not found.']);
            }

            return json_encode(['message' => 'Service item deleted successfully.']);
        } catch (Exception $e) {
            http_response_code(500);
            return json_encode(['error' => 'Database error: ' . $e->getMessage()]);
        }
    ...
    /**
     * Validates that the table name is one of the allowed service tables.
