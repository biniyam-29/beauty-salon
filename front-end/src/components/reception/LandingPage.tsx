...
import { useTodaysFollowUps } from "../../hooks/UseConsultations";
...
const navLinks: NavLinkItem[] = [
  { label: "Note a Phone Call", icon: <Phone />, path: "/reception/phone-book" },
  { label: "New Client", icon: <ClientPlusIcon />, path: "/reception/register" },
  { label: "Client Profiles", icon: <ClientListIcon />, path: "/reception/customers" },
  { label: "Daily Follow-ups", icon: <CalendarCheckIcon />, path: "/reception/reminders" },
];
...
const getRandomQuote = () => beautyQuotes[Math.floor(Math.random() * beautyQuotes.length)];
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive ? "bg-rose-100/60 text-rose-700" : "text-gray-600 hover:bg-rose-100/40 hover:text-rose-700"
      )}
    >
      <span className={cn(isActive ? "text-rose-600" : "text-gray-500")}>{item.icon}</span>
      {item.label}
    </Link>
  );
};
...
const TodaysSchedule: React.FC = () => {
  const { data: appointments = [], isLoading, isError } = useTodaysFollowUps();

  return (
    <div className="mt-12">
      <h3 className="text-xl font-semibold text-gray-800 mb-6">Today's Schedule</h3>
      <div className="bg-white rounded-2xl border border-rose-100/60 p-4 shadow-sm min-h-[200px]">
        <div className="max-h-96 overflow-y-auto pr-2">
          {isLoading ? (
            <ScheduleSkeleton />
          ) : isError ? (
            <p className="p-4 text-center text-red-500">Could not load today's schedule.</p>
            ...
            appointments.map((appt) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-semibold text-gray-700">{appt.customer_name}</span>
                </div>
                <span className="text-sm text-gray-500 bg-pink-50 px-3 py-1 rounded-full">
                  {appt.skin_concerns?.split(",")[0]?.trim() ?? "N/A"}
                </span>
              </div>
            ))
          ) : (
            <p className="p-12 text-center text-gray-500">No appointments scheduled for today.</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="w-full bg-[#FDF8F5] font-sans">
      <div className="flex h-screen">
        ...
        >
          <div onClick={() => setIsMobileMenuOpen(false)} className="absolute inset-0 bg-black/50" />
          <aside
            className={cn(
        ...
        <main className="w-full flex-1 p-6 sm:p-10 overflow-y-auto">
          <header className="flex justify-between items-center mb-10">
            ...
            <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden text-gray-700 p-2">
              <Menu size={28} />
            </button>
          </header>
