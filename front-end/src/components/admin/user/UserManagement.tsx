...
import { 
  ...
  ChevronLeft, ChevronRight, MoreVertical, Phone,
  Filter
} from 'lucide-react';
import { 
  useUsers,
...
const UserManagementView: React.FC = () => {
  ...
  const usersByRole = useMemo(() => {
    const counts: Record<UserRole, number> = {
      reception: 0, doctor: 0, admin: 0, cashier: 0, professional: 0,
    };
    
    users.forEach(user => {
      if (user.role in counts) counts[user.role as UserRole]++;
    });
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (isMenuOpen !== null && menuRef.current && !menuRef.current.contains(event.target as Node)) {
        setIsMenuOpen(null);
      }
    };
